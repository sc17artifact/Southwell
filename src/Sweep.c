...
int main(int argc, char *argv[])
{
   ...
   int arg_iter = 0;
   int m1 = 10, m2 = 10;
   double s_low = 0, s_high = 10, s_iter = 1;
   double tol = 1e-16;
   ...
   while(arg_iter < argc){
      ...
      else if (strcmp(argv[arg_iter], "-s_low") == 0){
         arg_iter++;
         s_low = atof(argv[arg_iter]);
      }
      else if (strcmp(argv[arg_iter], "-s_high") == 0){
         arg_iter++;
         s_high = atof(argv[arg_iter]);
      }
      else if (strcmp(argv[arg_iter], "-s_iter") == 0){
         arg_iter++;
         s_iter = atof(argv[arg_iter]);
      }
   ...
   unsigned long long sweep_obsvd = 0, sweep_obsvd_prev = -1;
   while (sweep <= s_high){
      if (solver_omp_aps){
         OMP_AsyncParSouthwell(A, x, b, &u, tol, sweep, &out_omp_aps);
         OMP_Residual(A, u, b, &r);
         r_norm = OMP_Norm2(r, n);
         t_obsvd = MeanDouble(out_omp_aps.t, threads);
         sweep_obsvd = MaxUL(out_omp_aps.sweep, threads);
         if ((r_norm_prev > r_norm) && (sweep_obsvd > sweep_obsvd_prev)){
      ...
      if (solver_omp_sps){
         OMP_SyncParSouthwell(A, x, b, &u, tol, sweep, &out_omp_sps);
         OMP_Residual(A, u, b, &r);
         r_norm = OMP_Norm2(r, n);
         t_obsvd = MeanDouble(out_omp_sps.t, threads);
         sweep_obsvd = MaxUL(out_omp_sps.sweep, threads);
       //  if ((r_norm_prev > r_norm) && (sweep_obsvd > sweep_obsvd_prev)){
      ...
      if (solver_omp_sj){
         OMP_SyncJacobi(A, x, b, &u, tol, sweep, &out_omp_sj);
         OMP_Residual(A, u, b, &r);
         r_norm = OMP_Norm2(r, n);
         t_obsvd = MeanDouble(out_omp_sj.t, threads);
         sweep_obsvd = MaxUL(out_omp_sj.sweep, threads);
       //  if ((r_norm_prev > r_norm) && (sweep_obsvd > sweep_obsvd_prev)){
            printf("%.10e %.10e %llu %llu\n",
      ...
      if (solver_omp_aj){
         OMP_AsyncJacobi(A, x, b, &u, tol, sweep, &out_omp_aj);
         OMP_Residual(A, u, b, &r);
         r_norm = OMP_Norm2(r, n);
         t_obsvd = MeanDouble(out_omp_aj.t, threads);
         sweep_obsvd = MaxUL(out_omp_aj.sweep, threads);
         if ((r_norm_prev > r_norm) && (sweep_obsvd > sweep_obsvd_prev)){
            printf("%.10e %.10e %llu %llu\n",
      ...
      if (solver_omp_mgs){
         OMP_MGS(A, x, b, &u, tol, sweep, &out_omp_mgs);
         OMP_Residual(A, u, b, &r);
         r_norm = OMP_Norm2(r, n);
         t_obsvd = MeanDouble(out_omp_mgs.t, threads);
         sweep_obsvd = MaxUL(out_omp_mgs.sweep, threads);
        // if ((r_norm_prev > r_norm) && (sweep_obsvd > sweep_obsvd_prev)){
            printf("%.10e %.10e %llu %llu\n",
      ...
      if (solver_seq_s){
         Southwell(A, x, b, &u, tol, sweep, &out_seq_s);
         OMP_Residual(A, u, b, &r);
         r_norm = OMP_Norm2(r, n);
         sweep_obsvd = MaxUL(out_seq_s.sweep, threads);
         if ((r_norm_prev > r_norm) && (sweep_obsvd > sweep_obsvd_prev)){
            printf("%.10e %.10e %llu %llu\n",
      ...
      if (solver_seq_gs){
         GaussSeidelRes(A, x, b, &u, tol, sweep, &out_seq_gs);
         OMP_Residual(A, u, b, &r);
         r_norm = OMP_Norm2(r, n);
         sweep_obsvd = MaxUL(out_seq_gs.sweep, threads);
        // if ((r_norm_prev > r_norm) && (sweep_obsvd > sweep_obsvd_prev)){
            printf("%.10e %.10e %llu %llu\n",
      ...
      if ((sweep_obsvd > sweep_obsvd_prev) && (r_norm < r_norm_prev)){
         t_obsvd_prev = t_obsvd;
         r_norm_prev = r_norm;
         sweep_obsvd_prev = sweep_obsvd;
      }
      sweep += s_iter;
   }
